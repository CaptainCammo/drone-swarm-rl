...
def create_scanning_waypoints():
    ...
    start_x, start_y, start_z = -250, -250, 50
    ...
    waypoints = [
        ...
        np.array([start_x + 400, start_y, start_z]),
...
def train_waypoint_agent(episodes=1000, mode='train', render=False, render_delay=0.01):
    """
    ...
    agent = DroneSwarmDQN(
        ...
        epsilon=1.0,
        epsilon_min=0.01,
        epsilon_decay=0.998,  # Slower epsilon decay for better exploration
        learning_rate=0.001
    )
    ...
    for episode in range(episodes):
        ...
        if mode == 'train' and (episode + 1) % 100 == 0:
            model_path = os.path.join('models', f'dqn_agent_episode_{episode+1}.pth')
            agent.save(model_path)
            print(f"Model saved to {model_path}")
    
    # Save final model
    if mode == 'train':
        model_path = os.path.join('models', 'dqn_agent_final.pth')
        agent.save(model_path)
...
def test_trained_agent(model_path, num_episodes=5):
    ...
    # Set epsilon to minimum for deterministic policy
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        train_waypoint_agent(episodes=args.episodes, render=True)
    else:
